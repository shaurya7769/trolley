...
CFG_PATH  = Path(**file**).parent / “rail_config.json”
_DEF = {
...
def **init**(self, title, current_val=“0”, decimals=1,
...
def **init**(self, title, presets=None, current=””, parent=None):
super().**init**(parent)
self.setWindowTitle(title)
self.setModal(True)
self.setStyleSheet(SS + “QDialog{background:#0e0e0e;}”)
...
```
    ...
    t = QLabel(title.upper())
    t.setStyleSheet(
        "color:{}; font-size:11pt; font-weight:bold; letter-spacing:2px;".format(AMBER)
    )
...
def **init**(self, color=NEON, parent=None):
super().**init**(parent)
self._d   = []
...
def push_error(self, msg):
    ...
    self._errbtn.setText("🔔  {} ERR".format(len(self._errs)))
    self._errbtn.setStyleSheet(
...
def __init__(self, key, title, unit, color, parent=None):
    ...
    self._alert.setStyleSheet(
        "color:{}; font-size:8pt; font-weight:bold; letter-spacing:2px;".format(RED))
...
def _do_save(self):
    ...
    save_cfg(self.cfg)
    self._info.setText("✓ CALIBRATED  |  scale={:.5f} mm/count".format(self._scale))
...
def _on_done(self, code, out):
    ...
    if self._phase == "zero":
        ...
        self._res.setText(
            "ADC: {}  Δ={}  mpc={:.5f} mm/count".format(val, delta, self._mpc))
        self._term.append(
            ("\n# {:.2f} mm / {} ADC-counts"
...
def _do_save(self):
    ...
    save_cfg(self.cfg)
    self._info.setText(
        "✓ CALIBRATED  |  zero={}  mpc={:.5f}".format(self._zero_raw, self._mpc))
    self._info.setStyleSheet("color:{}; font-size:9pt;".format(NEON))
    self._term.append("✓ Saved to rail_config.json")
...
def _on_done(self, code, out):
    ...
    if self._phase == "zero":
        self._offset = angle
        self._res.setText("Zero offset stored: {:.5f}°".format(angle))
        self._term.append(
            ("\n✓ Zero offset = {:.5f}°\n"
             "  Tap VERIFY to confirm correction is < ±0.05°.").format(angle))
        self._v_btn.setEnabled(True)
    elif self._phase == "verify":
        corr = angle - (self._offset or 0.0)
        ok   = abs(corr) < 0.05
        self._res.setText(
            "Corrected: {:.4f}°  ".format(corr)
            + ("✓ PASS (<0.05°)" if ok else "⚠  {:.4f}° — re-zero".format(corr)))
...
def _do_save(self):
    ...
    save_cfg(self.cfg)
    self._info.setText("✓ CALIBRATED  |  offset={:.5f}°".format(self._offset))
    self._info.setStyleSheet("color:{}; font-size:9pt;".format(NEON))
    self._term.append("✓ Saved to rail_config.json")
...
def _do_save(self):
    ...
    save_cfg(self.cfg)
    self._info.setText(
        "✓ CONFIGURED  |  ref={:.1f} m".format(self._ch_s.value()))
    self._info.setStyleSheet("color:{}; font-size:9pt;".format(NEON))
    self._term.append("✓ Saved to rail_config.json")
...
def _save(self):
    ...
    save_cfg(self.cfg)
    self._term.append("✓ Interface saved: {}".format(self._iface.value()))
```
...
def __init__(self, cfg):
    ...
    title.setStyleSheet(
        "color:{}; font-size:13pt; font-weight:bold; letter-spacing:3px;".format(AMBER))
...
def _sel(self, idx):
    for i, (btn, _, color) in enumerate(self._btns):
        if i == idx:
            btn.setStyleSheet(
                ("QPushButton{{ background:{}18; border:1px solid {};"
                 " border-radius:8px; color:{}; font-size:8pt;"
                 " font-weight:bold; padding:4px 8px; text-align:left;}}"
                 "QPushButton:pressed{{ background:{}28; }}").format(color, color, color, color))
        else:
            btn.setStyleSheet(
                "QPushButton{ background:#0c0c0c; border:1px solid #1e1e1e;"
                " border-radius:8px; color:#555; font-size:8pt;"
...
def __init__(self, label, color, freq_interval, unit, parent=None):
    ...
    self._hdr.setStyleSheet(
        ("QPushButton{{background:#0c0c0c; border:1px solid {}55;"
...
def __init__(self):
    ...
    title.setStyleSheet(
        "color:{}; font-size:13pt; font-weight:bold; letter-spacing:3px;".format(CYAN))
...
def __init__(self, parent=None):
    ...
    title.setStyleSheet(
        "color:{}; font-size:13pt; font-weight:bold; letter-spacing:3px;".format(MAGI))
    ...
    lbl.setStyleSheet(
        "color:{}; font-size:8pt; font-weight:bold; letter-spacing:2px;".format(MAGI))
    ll.addWidget(lbl)
    ...
    self._table.setStyleSheet((
        ...
        " padding:4px; }}"
        "QTableWidget::item:selected {{ background:{}33; color:#fff; }}"
        "QScrollBar:vertical {{ background:#0a0a0a; width:8px; }}"
        "QScrollBar::handle:vertical {{ background:#2a2a2a; border-radius:4px; }}"
...
self.encoder.sw_pressed.connect(self.*on_enc_sw)
self.encoder.start()
self.history    = {k: [] for k, ** in _METRICS}
